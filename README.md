...
Rather than reusing an existing account, create a dedicated user for the tunnel. Give it `/bin/false` as its shell — this allows SSH to establish the tunnel but prevents anyone from getting an interactive shell on the server, even if the private key were ever compromised:
 
```bash
sudo useradd -r -m -s /bin/false remotesshuser
```
 
The resulting `/etc/passwd` entry will look like:
 
```
remotesshuser:x:1002:1002:,,,:/home/remotesshuser:/bin/false
```
 
Add the Windows machine's public key to this user's `authorized_keys`:
 
```bash
sudo mkdir -p /home/remotesshuser/.ssh
sudo vi /home/remotesshuser/.ssh/authorized_keys
# paste the contents of id_ed25519.pub here
sudo chmod 700 /home/remotesshuser/.ssh
sudo chmod 600 /home/remotesshuser/.ssh/authorized_keys
sudo chown -R remotesshuser:remotesshuser /home/remotesshuser/.ssh
```
 
Use `remotesshuser@remote.host.on.internet` in all subsequent SSH commands instead of a regular user account.
 
### sshd_config
 
On `remotesshuser@remote.host.on.internet`, verify `/etc/ssh/sshd_config`:
